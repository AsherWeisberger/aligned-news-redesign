(function () {
  ...
  var DATA_URL = "live-data.json?v=an279";
  var NEWSLETTER_DATA_URL = "newsletter-data.json?v=an279";
  var state = {
    data: null,
    newsletter: [],
    filter: "all",
  ...
  function sourceCount(item) {
    return uniqueStorySources(item).length;
  }
  ...
  function storyFeedMeta(item) {
    var bits = [];
    var account = item.x_handle || item.author_name || xHandleFrom(item);
    if (account) bits.push(String(account).replace(/^@/, ""));
    if (item.published_at) bits.push(editionDate(item.published_at));
    var lists = Number(item.list_count) || (item.list_names || []).length;
    if (lists) bits.push(lists + (lists === 1 ? " list" : " lists"));
    return bits.join(" · ");
  }
  ...
  function editionDate(value) {
    ...
    return d.toLocaleDateString(window.anLoc ? window.anLoc() : undefined, {month: "short", day: "numeric", year: "numeric"});
  }

  function renderEdition(stories) {
    var meta = document.getElementById("editionMeta");
    if (!meta) return;
    var saved = getParam("view") === "saved";
    var stamp = state.data && state.data.meta && state.data.meta.generatedAt;
    if (saved) {
      meta.textContent = "Saved in this browser";
      return;
    }
    // Quiet edition line: date only (no Edition ·, no story counts)
    meta.textContent = stamp ? editionDate(stamp) : "";
  }

  function syncReaderUrl() {
    var url = new URL(location.href);
    if (state.filter !== "all") url.searchParams.set("section", state.filter); else url.searchParams.delete("section");
  ...
  function renderPullCard(s, i) {
    ...
    var headline = displayText(s.headline || s.title);
    var dek = uniqueDek(s, headline, 155);
    return (
      '<li class="story-card-shell top-pull" data-href="' + href + '" role="link" tabindex="0">' +
        '<p class="reader-story-label">' + escapeHtml(s.topic_label || s.section_label || s.section || "") + '</p>' +
        ...
        '<p class="reader-story-meta">' + escapeHtml(storyFeedMeta(s)) + '</p>' +
        storySaveBtnHtml(s) +
      "</li>"
    );
  }
  ...
  function uniqueDek(item, headline, maxLen) {
    maxLen = maxLen || 160;
    if (item.reader_summary) return displayText(item.reader_summary);
  ...
  function ensureLiveFooter() {
    ...
    siteFoot.innerHTML =
      ...
      "Coverage from Robert Scoble’s curated X lists · Static edition preview.</p>" +
  ...
  function renderChrome() {
    ...
    } else if (data.meta && data.meta.lastUpdatedTs) {
  ...
  function storyMatches(story) {
    ...
    if (state.query) {
      ...
      var hay = [story.headline, story.summary, story.body, story.section_label, story.section, story.signal_badge, story.author_name, story.x_handle]
        .join(" ").toLowerCase();
  ...
  function renderChips(containerId) {
    ...
    var allStories = (state.data.stories || []).filter(isTodayFeedKind);
    el.innerHTML = chips.map(function (c) {
      var count = c.id === "all" ? allStories.length : allStories.filter(function (story) { return storySectionKey(story) === c.id; }).length;
      var label = prettyChipLabel(c.id, c.label);
      return (
        '<button type="button" class="chip" data-filter="' + escapeHtml(c.id) + '" aria-pressed="' +
        (state.filter === c.id ? "true" : "false") + '" aria-label="' + escapeHtml(label + ", " + count + " stories") + '">' + escapeHtml(label) + "</button>"
      );
    }).join("");
  ...
  function renderTodayFeed() {
    ...
    function renderStoryItem(s, i, allowLead, rest) {
      ...
      if (!quiet && allowLead && showLead && i === 0) {
        var leadHeadline = displayText(s.headline || s.title);
        var dek = uniqueDek(s, leadHeadline, 240);
        ...
        return (
          '<li class="story-card-shell lead-card lead-card-opener lead-card-photo' + (isRead ? " is-read" : "") + '" style="--i:0" data-href="' + href + '" role="link" tabindex="0">' +
            ...
            '<div class="lead-after">' +
              ...
              '<a class="reader-open-story" href="' + href + '">Read the full story <span aria-hidden="true">→</span></a>' +
            "</div>" +
            storySaveBtnHtml(s) +
          "</li>"
        );
    ...
    if (isSaved) {
      ...
      sectionOrder.forEach(function (key) {
        ...
        html += '<li class="feed-rest-head" role="presentation"><h2 class="feed-rest-label">' + escapeHtml(prettyChipLabel(key, key)) + '</h2><a class="reader-see-all" href="index.html?section=' + encodeURIComponent(key) + '">View all ' + (bucket.length + items.filter(function(s) { return used[s.id] && storySectionKey(s) === key; }).length) + ' →</a></li>';
        bucket.slice(0, 3).forEach(function (s) {
  ...
  function renderTodayDeskModules() {
    ...
    if (sigHost) {
      var signals = ((state.data && state.data.signals) || []).slice(0, 3);
      if (!signals.length) {
        ...
        sigHost.innerHTML = signals.map(function (s) {
          ...
          var provenance = (s.list_names || []).length ? s.list_names.length + " curated lists" : "";
          var href = "story.html?id=" + encodeURIComponent("sigstory-" + s.id);
          return (
  ...
  function storyDeskRank(story) {
    if (Number(story.desk_rank) > 0) return Number(story.desk_rank);
  ...
  function renderStory() {
    ...
    var saved = isSavedId(story.id);
    var title = displayText(story.headline || story.title);
    var summary = displayText(story.summary || "").trim();
    ...
    var paras = storyBodyParagraphs(story);
    ...
    root.innerHTML =
      ...
      intelHtml +
      heroHtml +
      (sourceUrl ? '<a class="reader-source-action" href="' + escapeHtml(sourceUrl) + '" target="_blank" rel="noopener noreferrer">Open original source ↗</a>' : "") +
      ...
      "<div class=\"story-layout\"><div class=\"story-main\">" +
        "<section class=\"story-block story-explainer\"><h2>What happened</h2><div class=\"story-prose\">" + articleHtml + "</div></section>" +
        (whyIsDifferent ? "<section class=\"story-block story-explainer\"><h2>" + "Why it surfaced" + "</h2><div class=\"story-prose\"><p" + txSrc(why) + ">" + escapeHtml(why) + "</p></div></section>" : "") +
        ...
        originalHtml + usefulLinksHtml + relatedHtml +
  ...
  var DESK_TZ = "America/Indiana/Indianapolis";
  ...
  var DROPS_C6 = "#AD3308";
  var DROPS_C7 = "#FF7A24";
  var DESK_WINDOW_SEC = 4 * 3600;

  function deskDumpState() {
    var h = 0, m = 0, sec = 0;
    try {
      ...
      for (i = 0; i < parts.length; i++) {
        ...
        else if (p.type === "second") sec = parseInt(p.value, 10) || 0;
      }
    } catch (e) {
      var d = new Date();
    ...
    h = ((h % 24) + 24) % 24;
    var slot = 0, i;
    for (i = DESK_DUMP_HOURS.length - 1; i >= 0; i--) {
      if (h >= DESK_DUMP_HOURS[i]) { slot = i; break; }
    }
    var startH = DESK_DUMP_HOURS[slot];
    var elapsed = (h - startH) * 3600 + m * 60 + sec;
    if (elapsed < 0) elapsed = 0;
    if (elapsed > DESK_WINDOW_SEC) elapsed = DESK_WINDOW_SEC;
    var remain = DESK_WINDOW_SEC - elapsed;
    var progress = elapsed / DESK_WINDOW_SEC;
  ...
  function startDropsBar() {
    if (document.documentElement.getAttribute("data-drops") === "1") return;
    document.documentElement.setAttribute("data-drops", "1");
    var wrap = document.getElementById("anDropsBar");
    if (!wrap) {
      wrap = document.createElement("div");
      wrap.id = "anDropsBar";
      wrap.className = "an-drops-bar";
      wrap.setAttribute("role", "progressbar");
      wrap.setAttribute("aria-valuemin", "0");
      wrap.setAttribute("aria-valuemax", "100");
      var cv = document.createElement("canvas");
      cv.className = "an-drops-bar-canvas";
  ...
  function startMiniGlobe(canvas) {
    ...
    var FRAG = [
      ...
      "  float sphere=1.0-smoothstep(radius-edgeSoft,radius+edgeSoft,d);",
      "  vec2 p=uv*1.35;",
      "  vec2 q=p*3.1;",
      "  vec2 w1=vec2(fbm(q+vec2(t*0.21,-t*0.17)),fbm(q+vec2(3.1,8.4)-t*0.19));",
      "  vec2 w2=vec2(fbm(q*1.27-vec2(t*0.13,4.2)),fbm(q*0.83+vec2(9.2,-t*0.11)));",
      "  vec2 warped=p+(w1*0.42+w2*0.28);",
      "  float s1=fbm(warped*2.15+t*0.18);",
      "  float s2=fbm(warped*3.05-t*0.14+12.0);",
      "  float n=s1*0.58+s2*0.42;",
      "  vec3 phase=vec3(0.0,0.33,0.67);",
      "  vec3 pal=0.5+0.5*cos(6.2831853*(n*3.0+phase));",
      "  pal=mix(pal, pal*pal*(3.0-2.0*pal), 0.12);",
      "  vec3 nor=vec3(uv/max(radius,0.0001),0.0);",
      "  nor.z=sqrt(max(0.0,1.0-dot(nor.xy,nor.xy)));",
      "  nor=normalize(nor);",
      "  vec3 light=normalize(vec3(-0.62,0.58,0.52));",
      "  float diff=max(0.0,dot(nor,light));",
      "  float spec=pow(max(0.0,dot(nor,light)),32.0);",
      "  pal*=0.52+0.58*diff;",
      "  pal+=vec3(1.0)*spec*0.9;",
      "  pal=clamp(pal,0.0,1.0);",
      "  float limb=d-radius;",
      "  float pink=exp(-pow(max(limb,0.0)/0.085,2.0))*(1.0-sphere);",
      "  float blue=exp(-pow(max(limb,0.0)/0.175,2.0))*(1.0-sphere);",
      "  pink+=exp(-pow(min(limb,0.0)/0.04,2.0))*0.10*sphere;",
      "  vec3 glowPink=vec3(1.0,0.471,0.706);",
      "  vec3 glowBlue=vec3(0.353,0.627,1.0);",
      "  vec3 halo=glowBlue*blue*0.9+glowPink*pink;",
      "  vec3 col=pal*sphere+halo;",
      "  float alpha=min(1.0,max(sphere,max(pink,blue*0.92)));",
    ...
    function initWebgl() {
      try {
        gl = canvas.getContext("webgl", { alpha: true, premultipliedAlpha: false, antialias: true, depth: false, stencil: false }) ||
    ...
    function hash2(p, q) {
      var n = Math.sin(p * 127.1 + q * 311.7) * 43758.5453;
    ...
    function draw2d(w, time) {
      if (!ctx2) ctx2 = canvas.getContext("2d");
      if (!ctx2) return;
      if (!img2 || img2.width !== w) img2 = ctx2.createImageData(w, w);
      var data = img2.data;
      var x, y, i, uvx, uvy, d, sphere, pX, pY, qX, qY, w1x, w1y, w2x, w2y;
      var wx, wy, s1, s2, n, pr, pg, pb, nx, ny, nz, nl, lx, ly, lz, diff, spec;
      var limb, pink, blue, alpha, inv;
      var radius = 0.72;
      var invW = 2 / w;
      lx = -0.62; ly = 0.58; lz = 0.52;
      ...
      for (y = 0; y < w; y++) {
        for (x = 0; x < w; x++) {
          uvx = x * invW - 1;
          uvy = 1 - y * invW;
          d = Math.sqrt(uvx * uvx + uvy * uvy);
          sphere = 1 - Math.max(0, Math.min(1, (d - (radius - 0.005)) / 0.01));
          pX = uvx * 1.35; pY = uvy * 1.35;
          qX = pX * 3.1; qY = pY * 3.1;
          w1x = fbm2(qX + time * 0.21, qY - time * 0.17);
          w1y = fbm2(qX + 3.1 - time * 0.19, qY + 8.4);
          w2x = fbm2(qX * 1.27 - time * 0.13, qY * 1.27 - 4.2);
          w2y = fbm2(qX * 0.83 + 9.2, qY * 0.83 - time * 0.11);
          wx = pX + w1x * 0.42 + w2x * 0.28;
          wy = pY + w1y * 0.42 + w2y * 0.28;
          s1 = fbm2(wx * 2.15 + time * 0.18, wy * 2.15);
          s2 = fbm2(wx * 3.05 - time * 0.14 + 12, wy * 3.05);
          n = s1 * 0.58 + s2 * 0.42;
          pr = 0.5 + 0.5 * Math.cos(6.2831853 * (n * 3 + 0.00));
          pg = 0.5 + 0.5 * Math.cos(6.2831853 * (n * 3 + 0.33));
          pb = 0.5 + 0.5 * Math.cos(6.2831853 * (n * 3 + 0.67));
          nx = uvx / radius; ny = uvy / radius;
          nz = Math.sqrt(Math.max(0, 1 - nx * nx - ny * ny));
          inv = 1 / Math.sqrt(nx * nx + ny * ny + nz * nz + 1e-6);
          nx *= inv; ny *= inv; nz *= inv;
          diff = Math.max(0, nx * lx + ny * ly + nz * lz);
          spec = Math.pow(diff, 32);
          pr = Math.min(1, Math.max(0, pr * (0.52 + 0.58 * diff) + spec * 0.9));
          pg = Math.min(1, Math.max(0, pg * (0.52 + 0.58 * diff) + spec * 0.9));
          pb = Math.min(1, Math.max(0, pb * (0.52 + 0.58 * diff) + spec * 0.9));
          limb = d - radius;
          pink = Math.exp(-Math.pow(Math.max(limb, 0) / 0.085, 2)) * (1 - sphere);
          blue = Math.exp(-Math.pow(Math.max(limb, 0) / 0.175, 2)) * (1 - sphere);
          pink += Math.exp(-Math.pow(Math.min(limb, 0) / 0.04, 2)) * 0.10 * sphere;
          pr = pr * sphere + 0.353 * blue * 0.9 + 1.0 * pink;
          pg = pg * sphere + 0.627 * blue * 0.9 + 0.471 * pink;
          pb = pb * sphere + 1.0 * blue * 0.9 + 0.706 * pink;
          alpha = Math.min(1, Math.max(sphere, Math.max(pink, blue * 0.92)));
          i = (y * w + x) * 4;
          data[i] = pr * 255;
          data[i + 1] = pg * 255;
          data[i + 2] = pb * 255;
    ...
    glMode = initWebgl();
    canvas.dataset.gmGl = glMode ? "1" : "0";

    function sizeToCss() {
      var dpr = Math.min(2, window.devicePixelRatio || 1);
      var css = canvas.clientWidth || 32;
      var size = Math.max(32, Math.round(css * dpr));
      if (canvas.width !== size || canvas.height !== size) {
        canvas.width = size;
        canvas.height = size;
        img2 = null;
        if (glMode && gl) gl.viewport(0, 0, size, size);
      }
      return size;
    }
    ...
    function draw(ts) {
      if (!running) return;
      if (!t0) t0 = ts || 0;
      var time = reduce ? freezeT : (((ts || 0) - t0) / 1000) * 1.5;
      var size = sizeToCss();
      if (glMode && gl && prog) {
